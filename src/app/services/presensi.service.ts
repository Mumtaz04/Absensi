...
checkIn(latitude: number, longitude: number): Observable<any> {
  ...
  return this.http.post(`${this.apiUrl}/check-in`, body, { headers: this.getHeaders() })
    .pipe(
      catchError(err => {
...
checkOut(latitude: number, longitude: number): Observable<any> {
  ...
  const body = { latitude, longitude };
  return this.http.post(`${this.apiUrl}/check-out`, body, { headers: this.getHeaders() })
    .pipe(
      catchError(err => {
  ...
  getRiwayatPresensi(): Observable<any> {
    return this.http.get(`${this.apiUrl}/attendances/history`, { headers: this.getHeaders() })
      .pipe(
        catchError(err => {
